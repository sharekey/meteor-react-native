...
class DDP extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.isVerbose = options.isVerbose ?? true;
    ...
    if (this.isVerbose) {
        this.socket.on('message:out', (outMessage) => {
          try {
            ...
            if (copy.params !== undefined) delete copy.params;
            this.logger(copy);
        } catch (e) {
          // no-op
        }
      ...
      if (message.msg === 'connected') {
        ...
        if (this.isVerbose) {
          if (message.msg === 'ready' || message.msg === 'nosub' || message.msg === 'error') {
            ...
            this.logger({ msg: message.msg, collection: message.collection, id: message.id });
          } else {
            this.logger(message);
          }
        }
