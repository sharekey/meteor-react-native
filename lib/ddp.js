...
class DDP extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.socket.on('open', () => {
      ...
      // Attempt to resume previous DDP session if available
    ...
    this.socket.on('message:in', (message) => {
      if (message.msg === 'connected') {
        ...
        console.info(`DDP ${sessionReused ? 'reused' : 'new'} session established. OLD: ${previousSessionId}, NEW: ${message.session}`, message);
